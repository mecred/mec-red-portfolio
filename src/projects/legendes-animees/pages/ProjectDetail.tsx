        ...
        {isLoading ? (
          ...
          >
            ...
            <div className="px-10 mb-20 flex justify-between items-center relative z-20">
              <Link to="/legendes-animees" className="inline-flex items-center gap-4 group opacity-50 hover:opacity-100 transition-opacity" style={{ color: theme.accent }}>
            ...
            <section className="px-10 mb-60">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-6xl mx-auto">
                    {PROJECTS.filter(p => p.id !== project.id && ((p.region && p.region === project.region) || (p.location && project.location && p.location.includes(project.location.split(',')[0])))).slice(0, 4).map(related => (
                       <Link key={related.id} to={`/legendes-animees/project/${related.slug}`} className="group relative aspect-square overflow-hidden border border-white/10 bg-white/5 block">
            ...
            <section className="bg-white text-black py-60 px-10 relative overflow-hidden flex flex-col items-center text-center">
               ...
               <Link to={`/legendes-animees/project/${nextProject.slug}`} className="group">
                 <motion.h2 
